...
#define ZYNewfeatureImageCount 3
...
- (void)setupPageControl
{
    ...
    pageControl.numberOfPages = ZYNewfeatureImageCount;
...
- (void)setupScrollView
{
    ...
    for (int index = 0; index<ZYNewfeatureImageCount; index++) {
        ...
        // 在最后一个图片上面添加按钮
        if (index == ZYNewfeatureImageCount - 1) {
            [self setupLastImageView:imageView];
        }
    }
    
    // 3.设置滚动的内容尺寸
    scrollView.contentSize = CGSizeMake(imageW * ZYNewfeatureImageCount, 0);
...
- (void)start
{
    ...
    // 切换窗口的根控制器
    self.view.window.rootViewController = [[ZYTabBarController alloc] init];
}
